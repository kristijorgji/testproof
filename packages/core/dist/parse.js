...
import { findPlatform, inferAreaScope, ledgerPlatforms, scopeToTargets, targetDimensions, targetPlatformId, } from './platforms.js';
import { DEFAULT_PLATFORMS, FLOW_ID_RE, ledgerSchema, } from './schema.js';
export function flattenFlows(ledger) {
    const out = [];
    const walk = (flows) => {
...
export function flattenFlowIds(ledger) {
    return flattenFlows(ledger).map((f) => f.id);
}
export function attachImplicitTargets(ledger) {
    const walk = (flows, area) => flows.map((flow) => ({
        ...flow,
...
function assertRefs(ledger) {
    ...
    const checkFlow = (flow, area) => {
        ...
        for (const target of flow.targets ?? []) {
            ...
            for (const [dimId, values] of Object.entries(dims)) {
                ...
                if (!dim)
                    throw new Error(`flows ledger: unknown dimension "${dimId}" on ${flow.id}`);
                if (dim.appliesTo?.length && !dim.appliesTo.some((p) => platformId === p || platformId.startsWith(`${p}.`))) {
                    throw new Error(`flows ledger: dimension "${dimId}" does not apply to platform "${platformId}" on ${flow.id}`);
                }
    ...
    for (const id of flattenFlowIds(ledger)) {
        if (seen.has(id))
            throw new Error(`flows ledger: duplicate FLOW id "${id}"`);
        seen.add(id);
    }
    const platformIds = new Set();
    const walkP = (nodes) => {
        for (const node of nodes) {
            if (platformIds.has(node.id))
                throw new Error(`flows ledger: duplicate platform id "${node.id}"`);
...
export function parseLedger(yamlSource) {
