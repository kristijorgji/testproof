...
function fileLinks(files) {
    if (files.length === 0)
        return '<span class="muted">todo</span>';
    return files
        .map((f) => {
...
function renderFlowHtml(flow, coverage, depth) {
...
export function renderFlowsHtml(ledger, coverage, options = {}) {
...
<section class="area" data-area="${escapeHtml(area.id)}" data-scope="${area.scope ?? 'common'}">
  ...
  ${groups}
        ...
        .join('');
    const statuses = ['automated', 'partial', 'todo', 'manual'];
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
