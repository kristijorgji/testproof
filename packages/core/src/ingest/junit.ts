...
export function parseJunitXml(xml: string, platform = 'web'): RunResult[] {
    ...
    for (const block of cases) {
        ...
        if (/<failure\b/.test(block) || /<error\b/.test(block)) status = 'fail';
        else if (/<skipped\b/.test(block)) status = 'skip';
        ...
        if (unique.length === 0) results.push({ flowId: null, ...entry });
        else for (const flowId of unique) results.push({ flowId, ...entry });
    }
    return results;
}
