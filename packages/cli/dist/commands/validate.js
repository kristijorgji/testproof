...
import { validateLedger } from '@testproof/core';
export function validateCommand(config, cwd, strict = false) {
    ...
    if (result.missingFromLedger.length > 0) {
        ...
        for (const id of result.missingFromLedger)
            console.error(`  - ${id}`);
        return 1;
    }
    if (result.incompleteCoreIds.length > 0) {
        ...
        if (strict) {
            console.error(line);
            for (const id of result.incompleteCoreIds)
                console.error(`  - ${id}`);
            return 1;
        }
        console.warn(line);
    ...
    console.log(`testproof validate: ok (maestro=${result.maestroIds.length} web=${result.webIds.length} ledger=${result.ledgerIds.length})`);
    return 0;
}
